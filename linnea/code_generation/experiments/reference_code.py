...
def generate_reference_code(output_name, equations):
    ...
    utils.algorithm_to_file(output_name, "reference", "naive_blaze", equations.to_cpp_expression(config.CppLibrary.Blaze),
                          input_str, output_str, config.Language.Cpp, ".hpp")
    utils.algorithm_to_file(output_name, "reference", "naive_eigen", equations.to_cpp_expression(config.CppLibrary.Eigen),
                          input_str, output_str, config.Language.Cpp, ".hpp")
    utils.algorithm_to_file(output_name, "reference", "naive_armadillo", equations.to_cpp_expression(config.CppLibrary.Armadillo),
                          input_str, output_str, config.Language.Cpp, ".hpp")

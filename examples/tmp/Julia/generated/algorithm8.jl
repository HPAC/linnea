...
"""
    algorithm8(ml0::Array{Float64,2}, ml1::Array{Float64,1})
...
"""                    
function algorithm8(ml0::Array{Float64,2}, ml1::Array{Float64,1})
    # cost: 3.34e+09 FLOPs
    ...
    # X: ml0, full, y: ml1, full, tmp1: ml2, full
    # (L2 L2^T) = tmp1
    LAPACK.potrf!('L', ml2)

    # X: ml0, full, y: ml1, full, L2: ml2, lower_triangular
    ml3 = Array{Float64}(undef, 1000)
    # tmp8 = (X^T y)
    gemv!('T', 1.0, ml0, ml1, 0.0, ml3)

    # L2: ml2, lower_triangular, tmp8: ml3, full
    # tmp10 = (L2^-1 tmp8)
    trsv!('L', 'N', 'N', ml2, ml3)
